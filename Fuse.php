...
class Fuse
{
    ...
    */
    ...
    {
        return preg_replace_callback("/\@For ?\((.*)\)((.|\n)*)\@endFor/", function ($matches) {return "<?php for ($matches[1]): ?>$matches[2]<?php endfor; ?>";}, $input);
    }

    private function parseWhile(string $input)
    {
        return preg_replace_callback("/\@While ?\((.*)\)((.|\n)*)\@endWhile/", function ($matches) {return "<?php while ($matches[1]): ?>$matches[2]<?php endwhile; ?>";}, $input);
    }

    private function parseIf(string $input)
    {
        return preg_replace_callback("/\@If ?\((.*)\)((.|\n)*)\@endIf/", function ($matches) {
            $matches[2] = preg_replace_callback("/\@Else\b/", function () {return '<?php else: ?>';}, $matches[2]);
            $matches[2] = preg_replace_callback("/\@ElseIf ?\((.*)\)/", function ($m) {return "<?php elseif ($m[1]): ?>";}, $matches[2]);
